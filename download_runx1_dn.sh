...
macs_14_atac=true #For ATAC-seq MACS14 set macs_14_chip=true
...
sed -i -e 's,Rag2 -/- ,DN; ,g' $infodir/sample_files_${des}.txt
sed -i -e '/H3K/d' $infodir/sample_files_${des}.txt
sed -i -e 's/GSM1360737: DP E47; Mus musculus; ChIP-Seq/GSM1360737,DP,Mus musculus,ChIP_seq,E47,Cauchy,ABI,Single,Rep_0/g' $infodir/sample_files_${des}.txt
sed -i -e 's/GSM1360736: DN; Input; Mus musculus; ChIP-Seq/GSM1360736,DN,Mus musculus,ChIP_seq,Input,Cauchy,Illumina,Single,Rep_0/g' $infodir/sample_files_${des}.txt
sed -i -e 's/GSM1360735: DN; Runx1; Mus musculus; ChIP-Seq/GSM1360735,DN,Mus musculus,ChIP_seq,Runx1,Cauchy,ABI,Single,Rep_0/g' $infodir/sample_files_${des}.txt
sed -i -e 's/GSM1360719: DN; Ets1 ChIP-Seq biorep 2; Mus musculus; ChIP-Seq/GSM1360719,DN,Mus musculus,ChIP_seq,Ets1,Cauchy,Illumina,Single,Rep_2/g' $infodir/sample_files_${des}.txt
sed -i -e 's/GSM1360718: DN; Ets1 ChIP-Seq biorep 1; Mus musculus; ChIP-Seq/GSM1360718,DN,Mus musculus,ChIP_seq,Ets1,Cauchy,Illumina,Single,Rep_1/g' $infodir/sample_files_${des}.txt
...
sed ' ' $paralleldir/macs_parallel_${des}.txt | grep E47 > $paralleldir/macs_parallel_atac_${des}.txt
sed ' ' $paralleldir/macs_parallel_${des}.txt | grep '156\|142' > $paralleldir/macs_parallel_chip_${des}.txt
sed -i -e 's, 156, 156 /mnt/data1/John/Pioneer_Factors/Data/ChIP_seq/DN/Input/Cauchy/DN_ChIP_seq_Input_Cauchy_mm10_GSM1360736.bam,g' $paralleldir/macs_parallel_chip_${des}.txt
sed -i -e 's, 142, 142 /mnt/data1/John/Pioneer_Factors/Data/ChIP_seq/DN/Input/Cauchy/DN_ChIP_seq_Input_Cauchy_mm10_GSM1360736.bam,g' $paralleldir/macs_parallel_chip_${des}.txt
...
echo "Notes: E47 in DP, may not be good quality as only 20% of reads aligned. Ets1 and Runx1 in Rag2-/- thymocytes (which are effectively DN). Reads are high quality and with no adapter contamination." >> $logdir/download_log_${des}.txt 
echo "-------------------------------------------------------------------------------------------------------------------------------------" >> $logdir/download_log_${des}.txt  
