...
#Once the SAMPLE FILE is properly FORMATTED, set test1=true
test1=true
#Step 4: Check to make sure the SRR FILE is properly formatted
#Once the SRR FILE is properly FORMATTED, set test2=true
test2=true
#To CARRY OUT the download, set download=true
#Once DOWNLOADED, set download=false to avoid redownloading it.
download=false
...
#Once files are put into the right place, set replicate_copy=true
replicate_copy=false
...
macs_14_atac=true #For ATAC-seq MACS14 for single-end data, set macs_14_chip=true
...
#For MACS2 Peak Calling, set macs_2=true
macs_2=false
#Check to make sure the MACS2 commands file is correct (below) before setting an option and CARRYING OUT the peak calling
...
atacadapter=true #Do adapter/quality trimming for ATAC seq datasets
chipadapter=false #Do adapter/quality trimming for ChIP seq datasets
...
sed -i -e 's/GSM1978248: ATAC-seq Beta3; Homo sapiens; OTHER/GSM1978248,Beta,Homo sapiens,ATAC_seq,Wang,Illumina,Paired,Rep_0/g' $infodir/sample_files_${des}.txt
sed -i -e 's/GSM1978247: ATAC-seq Beta2; Homo sapiens; OTHER/GSM1978247,Beta,Homo sapiens,ATAC_seq,Wang,Illumina,Paired,Rep_0/g' $infodir/sample_files_${des}.txt
sed -i -e 's/GSM1978246: ATAC-seq Beta1; Homo sapiens; OTHER/GSM1978246,Beta,Homo sapiens,ATAC_seq,Wang,Illumina,Paired,Rep_0/g' $infodir/sample_files_${des}.txt
sed -i -e 's/GSM1978245: ATAC-seq Alpha3; Homo sapiens; OTHER/GSM1978245,Alpha,Homo sapiens,ATAC_seq,Wang,Illumina,Paired,Rep_0/g' $infodir/sample_files_${des}.txt
sed -i -e 's/GSM1978244: ATAC-seq Alpha2; Homo sapiens; OTHER/GSM1978244,Alpha,Homo sapiens,ATAC_seq,Wang,Illumina,Paired,Rep_0/g' $infodir/sample_files_${des}.txt
sed -i -e 's/GSM1978243: ATAC-seq Alpha1; Homo sapiens; OTHER/GSM1978243,Alpha,Homo sapiens,ATAC_seq,Wang,Illumina,Paired,Rep_0/g' $infodir/sample_files_${des}.txt
...
function adapter-trim {
  ...
  if [[ "$type" = "Paired" ]]; then
    ...
    trim_galore -o `pwd` --${adapter} `pwd`/${srr}_1.fastq
    mv ${srr}_1_trimmed.fq ${srr}_1.fastq
    echo "trim_galore -o `pwd` --${adapter} `pwd`/${srr}_2.fastq" >> $logdir/download_log_${des}.txt
    trim_galore -o `pwd` --${adapter} `pwd`/${srr}_2.fastq
...
rm -f $paralleldir/macs_parallel_input_${des}.txt
...
if [[ $macs_14_atac = true ]]; then
  ...
  if [ -f $paralleldir/macs_parallel_atac_${des}.txt ]; then
    ...
    parallel --xapply --dryrun -j 35 --colsep ' ' -a $paralleldir/macs_parallel_atac_${des}.txt "macs14 -t {1} -n {2} --bw {3} -f BAM -g hs -p $MACSpvalue -w --single-profile >> {2}.log 2>&1" >> $logdir/download_log_${des}.txt
    echo "#--------------------------------------------------------" >> $logdir/download_log_${des}.txt
    parallel --xapply --dryrun -j 35 --colsep ' ' -a $paralleldir/macs_parallel_atac_${des}.txt "macs14 -t {1} -n {2} -f BAM -g hs -p $MACSpvalue -w --single-profile >> {2}.log 2>&1"
    parallel --xapply -j 35 --colsep ' ' -a $paralleldir/macs_parallel_atac_${des}.txt "macs14 -t {1} -n {2} -f BAM -g hs -p $MACSpvalue -w --single-profile >> {2}.log 2>&1"
  fi
fi #macs_14_atac
...
if [[ $macs_14_chip = true ]]; then
  ...
  if [ -f $paralleldir/macs_parallel_input_${des}.txt ]; then
    echo "#-------------------MACS PEAK CALLING--------------------------" >> $logdir/download_log_${des}.txt
    parallel --xapply --dryrun -j 35 --colsep ' ' -a $paralleldir/macs_parallel_input_${des}.txt "macs14 -t {1} -n {2} --bw {3} -f BAM -g mm -p $MACSpvalue -w --single-profile >> {2}.log 2>&1" >> $logdir/download_log_${des}.txt
    echo "#--------------------------------------------------------" >> $logdir/download_log_${des}.txt
    parallel --xapply --dryrun -j 35 --colsep ' ' -a $paralleldir/macs_parallel_input_${des}.txt "macs14 -t {1} -n {2} --bw {3} -f BAM -g mm -p $MACSpvalue -w --single-profile >> {2}.log 2>&1"
    parallel --xapply -j 35 --colsep ' ' -a $paralleldir/macs_parallel_input_${des}.txt "macs14 -t {1} -n {2} --bw {3} -f BAM -g mm -p $MACSpvalue -w --single-profile >> {2}.log 2>&1"
  fi
fi #macs_14_chip 
...
repnum=$(echo $line | cut -d',' -f8 | cut -d'_' -f2)
if [[ "$seq" = ATAC* ]]; then
  cd $dir0/Data/ATAC_seq ; cd $cell/$investigator ; cd macs
  filename=${cell}_${seq}_${investigator}_${genome}_${gsm}
fi
if [[ "$seq" = ChIP* ]]; then
  seq=$(echo $line | cut -d',' -f4)
...
if [[ "$seq" = ATAC* ]]; then
  ...
  if [ -f ${filename}_treat_afterfiting_all.wig.gz ]; then
    ...
    if [[ "$genome" = mm10 ]]; then
      ...
      wigToBigWig ${filename}_normalized.wig $datadir/hg19.chrom.sizes ${filename}_normalized.bw -clip     
    fi
...
if [[ "$seq" = ATAC* ]]; then
  ...
  if [[ "$genome" = hg* ]]; then
    ...
    cp ${filename}.bedgraph ${filename}_sort.bedgraph
    sed -n '/chr10	/p' ${filename}.bedgraph  > ${filename}_sort.bedgraph
    sed -n '/chr11	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph
    sed -n '/chr1	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph   
    sed -n '/chr12	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph  
    sed -n '/chr13	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr14	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr15	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr16	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr17	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr18	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr19	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr20	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr21	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr22	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr2	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr3	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr4	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr5	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr6	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr7	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr8	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chr9	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chrX	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
    sed -n '/chrY	/p' ${filename}.bedgraph  >> ${filename}_sort.bedgraph 
  
      #| sort -k1,1 -k2,2n | sed -e 's/chr1A/chr1/' > ${filename}_sort.bedgraph 
    #sed -e 's/chr1	/chr1A	/' ${filename}_sort.bedgraph  | sort -k1,1 -k2,2n | sed -e 's/chr1A/chr1/' > ${filename}_sort.bedgraph 
    #sed ' ' ${filename}.bedgraph > ${filename}_sort.bedgraph    
    bedtools coverage -sorted -g $datadir/sorted.hg19.genome -counts -a ${filename}_sort.bedgraph  -b ../${filename}.bam  > ${filename}_tag_peaks.bedgraph #awk 'BEGIN{OFS="\t"} {print $1, $2, $3, $5}' - > ${filename}_tag_peaks.bedgraph
  elif [[ "$genome" = mm10 ]]; then
    #---------------------CREATE BEDGRAPH FROM PEAKS------------------------------------------
    ##This generates a bedgraph of peaks using number of tags from MACS output using the generated xls file 
    $scriptdir/Macs2Bedgraph ${filename}_peaks.xls > ${filename}.bedgraph	##Custom script to append columns of CHR, Start and End to number of tags from xls file
...
if [[ "$seq" = ChIP* ]]; then
  ...
  if echo "$mark" | grep -q Input; then
    ...
    if [[ "$genome" = hg* ]]; then
      ...
      #sed -i '/_random/d' ${filename}.bedgraph
      sed -e 's/chr1	/chr1A	/' ${filename}.bedgraph  | sort -k1,1 -k2,2n | sed -e 's/chr1A/chr1/' > ${filename}_sort.bedgraph 
      bedtools coverage -sorted -g $datadir/sorted.hg19.genome -counts -a ${filename}_sort.bedgraph  -b ../${filename}.bam | awk 'BEGIN{OFS="\t"} {print $1, $2, $3, $5}' - > ${filename}_tag_peaks.bedgraph
    elif [[ "$genome" = mm10 ]]; then
      #---------------------CREATE BEDGRAPH FROM PEAKS------------------------------------------
      ##This generates a bedgraph of peaks using number of tags from MACS output using the generated xls file 
      $scriptdir/Macs2Bedgraph ${filename}_peaks.xls > ${filename}.bedgraph	##Custom script to append columns of CHR, Start and End to number of tags from xls file
...
if [[ $macs_2 = true ]]; then
echo "Doing peak calling with MACS 2"
##--------------MACS 2 OUTPUT COMMANDS COMBINED BAM FILE-------------------------------
#Read in a file and output commands to a file for MACS 2 Peak Calling
lines=$(wc -l $infodir/sample_files_${des}.txt  | cut -d' ' -f1)
for ((i=1; i<=$lines; i++)); do
line=$(sed -n "${i}p" < $infodir/sample_files_${des}.txt )
gsm=$(echo $line | cut -d',' -f1)
...
repnum=$(echo $line | cut -d',' -f8 | cut -d'_' -f2)
if [[ "$seq" = ATAC* ]]; then
  cd $dir0/Data/ATAC_seq ; cd $cell/$investigator ; mkdir -p macs2
  filename=${cell}_${seq}_${investigator}_${genome}_${gsm}
...
if [[ "$seq" = ChIP* ]]; then
  ...
  if echo "$mark" | grep -q Input; then
    ...
    cd $dir0/Data/ChIP_seq ; cd $cell/$mark/$investigator ; mkdir -p macs2
    echo "`pwd`/${filename}.bam `pwd`/macs/${filename}">> $paralleldir/macs_parallel_${des}.txt
  fi
fi
if [[ "$seq" = RNA* ]]; then
...
echo ""
echo "#------------------CHECK MACS2 PARALLEL COMMANDS------------------------"
sed '' $paralleldir/macs_parallel_${des}.txt
echo "#--------------------------------------------------------"
echo ""
echo "#--------------------------------------------------------"
echo ""
echo "-----------------CHECK MACS2 ATAC Seq PARALLEL COMMANDS-----------------"
sed '' $paralleldir/macs_parallel_atac_${des}.txt
echo "#--------------------------------------------------------"
echo "-----------------CHECK MACS2 ChIP Seq PARALLEL COMMANDS-----------------"
sed '' $paralleldir/macs_parallel_chip_${des}.txt
echo "#--------------------------------------------------------"
echo ""
echo "-----------------CHECK MACS2 ChIP Seq Input PARALLEL COMMANDS-----------------"
sed '' $paralleldir/macs_parallel_input_${des}.txt
echo "#--------------------------------------------------------"
echo ""
echo "Check to make sure MACS2 commands are correct before setting the next option"
echo "Format should be PATH_TO_BAM OUTPUT_NAME fragLength Input"
echo "Transcription factor ChIP-seq should have the full path to the Input file added to the end of each line (separated with a space)"
echo "Set macs_2_chip = true if sample files are transcription factor ChIP-seq with an Input file"
echo "Set macs_2_atac = true if sample files do not have an Input file"

#---------------------MACS 2 PEAK CALLING---------------------------------------------------
##This uses MACS 2 to determine ATAC_seq peaks, mostly used for paired-end data
#-----------------------------------------------------------------------------------------
MACSpvalue=1e-7

##----------------------MACS 2 Peak Calling ATAC seq----------------------------------------
if [[ $macs_2_atac = true ]]; then
  echo ""; echo "--------------MACS Peak Calling ATAC seq---------------"
  if [ -f $paralleldir/macs_parallel_atac_${des}.txt ]; then
    echo "#-------------------MACS PEAK CALLING--------------------------" >> $logdir/download_log_${des}.txt
    parallel --xapply --dryrun -j 35 --colsep ' ' -a $paralleldir/macs_parallel_atac_${des}.txt "macs2 callpeak -t {1} -n {2} -f BAMPE -g hs -B -p $MACSpvalue >> {2}.log 2>&1" >> $logdir/download_log_${des}.txt
    echo "#--------------------------------------------------------" >> $logdir/download_log_${des}.txt
    parallel --xapply --dryrun -j 35 --colsep ' ' -a $paralleldir/macs_parallel_atac_${des}.txt "macs2 callpeak -t {1} -n {2} -f BAMPE -g hs -B -p $MACSpvalue >> {2}.log 2>&1"
    #parallel --xapply -j 35 --colsep ' ' -a $paralleldir/macs_parallel_atac_${des}.txt "macs2 callpeak -t {1} -n {2} -f BAMPE -g hs -B -p $MACSpvalue >> {2}.log 2>&1"
  fi
fi #macs_2_atac

##----------------------MACS 2 Peak Calling ChIP seq----------------------------------------
if [[ $macs_14_chip = true ]]; then
  echo ""; echo "--------------MACS Peak Calling ChIP seq---------------"
  if [ -f $paralleldir/macs_parallel_chip_${des}.txt ]; then
    echo "#-----------------MACS PEAK CALLING--------------------------" >> $logdir/download_log_${des}.txt
...
if [[ "$seq" = ATAC* ]]; then
  ...
  if [[ "$sequencer" = Illumina ]]; then
    if [[ "$type" = Paired ]]; then
      echo "Skipping copying of fastq file to data dir because paired-end"
    elif [[ "$type" = Single ]]; then
...
echo "Notes: Good quality datasets. " >> $logdir/download_log_${des}.txt 
echo "-------------------------------------------------------------------------------------------------------------------------------------" >> $logdir/download_log_${des}.txt  
